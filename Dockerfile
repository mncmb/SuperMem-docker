...
RUN git clone https://github.com/CrowdStrike/SuperMem.git /app && cd /app && pip3 install -r requirements.txt && chmod +x ./winSuperMem.py && echo -e 'python3 /app/winSuperMem.py "$@"' > /usr/bin/winSuperMem && chmod +x /usb/bin/winSuperMem && mkdir /work
...
RUN wget https://bootstrap.pypa.io/pip/2.7/get-pip.py && python ./get-pip.py && pip install distorm3 pycrypto
